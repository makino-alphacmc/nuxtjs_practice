...
2. **コンポーネントの分割** - `components/crud/p1/PostEditForm.vue` で再利用性・保守性を向上
...
```typescript
import type { UpdatePostRequest, Post } from '~/types/crud/p1/api'
...
import type { Post, UpdatePostRequest } from '~/types/crud/p1/api'
...
3. **コンポーネントを作成**: `components/crud/p1/PostEditForm.vue` で UI を分割

...
1. **型定義の明確化** – `Post` の `id` を `number` 型として定義
2. **コンポーネント分割** – 削除ボタンを含む一覧 UI を `PostList.vue` に切り出し、親はロジックに集中（props で受け取る）
3. **ロジックの分離** – `useHttpPosts()` に DELETE リクエストロジックを集約
...
- **型定義の明確化**: `Post` の `id` を `number` 型として定義してから実装を始める
- **ロジックの分離**: DELETE リクエストは composable に集約し、UI はボタン操作のみ（エラー/ローディング処理を必ず含める）
- **コンポーネント分割**: composable に処理を閉じ込めることでページやコンポーネントはシンプルになる（props で受け取る）
- **削除確認**: 削除は不可逆的な操作のため、必ず確認ダイアログを表示する

このセクションでは、DELETE リクエストの基本的な使い方と、実務で必須の 4 つの概念（型定義・コンポーネント分割・ロジックの分離・明示的なインポート）を学びました。これで CRUD 操作（Create, Read, Update, Delete）の全てを実装できました。

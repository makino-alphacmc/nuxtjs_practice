...
2. **コンポーネント分割** – 入力フォームと結果表示を分け、役割を明確化（props で受け取る）
...
- **コンポーネント分割**: フィールド単位の表示と、全体一覧表示の両方を実装（props で受け取る）
- **ロジックの分離**: エラー情報は props で渡し、UI では表示だけに専念
- **UI/UX**: Tailwind で色と余白を統一し、視認性を高める

このセクションでは、エラー表示でフィードバックする基本的な使い方と、実務で必須の 4 つの概念（型定義・コンポーネント分割・ロジックの分離・明示的なインポート）を学びました。次のセクションでは、サブミット処理（送信）の実装方法を学びます。

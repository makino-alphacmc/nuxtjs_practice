...
2. **コンポーネント分割** – 結果表示は `SubmitResultCard` に任せ、メインフォームをシンプルに（props で受け取る）
3. **ロジックの分離** – サブミット処理は composable に集約し、UI はボタン操作のみ
...
- **ロジックの分離**: サブミットは「バリデーション → ローディング → 非同期処理 → 結果反映 → ローディング解除」の順（composable に集約）
- **コンポーネント分割**: composable に処理を閉じ込めることでページやコンポーネントはシンプルになる（props で受け取る）
- **実務対応**: 実務/API 置き換えも容易（エラー/ローディング処理を必ず含める）

このセクションでは、サブミット処理（送信）の基本的な使い方と、実務で必須の 4 つの概念（型定義・コンポーネント分割・ロジックの分離・明示的なインポート）を学びました。次のセクションでは、フォームリセットの実装方法を学びます。
